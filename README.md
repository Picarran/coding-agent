...
前端用 **Vite + Vue 3** 构建（白底、深色为辅的现代 UI）：

```bash
# 1. 构建前端（产出 web/frontend/dist/）
cd web/frontend
npm install
npm run build
cd ../..
...
> 注：浏览器原生文件夹选择弹窗拿不到绝对路径（File System Access API 只给 opaque handle），而 agent 需在服务器侧读写真实文件，所以「添加工作区」用的是服务端目录浏览器（`GET /api/fs/list`）。
