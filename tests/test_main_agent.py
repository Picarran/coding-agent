"""Tests for the Main Agent's Plan-and-Execute loop (fake planner/replanner/worker)."""
...
class MainAgentTest(unittest.TestCase):
    def test_executes_all_steps_in_order(self):
        ...
        agent = MainAgent(FakePlanner(plan), FakeReplanner(plan), worker)
        result = agent.run("g")
    ...
    def test_replans_on_failure(self):
        ...
        agent = MainAgent(FakePlanner(plan), FakeReplanner(retry_plan), worker, max_replans=1)
        result = agent.run("g")
